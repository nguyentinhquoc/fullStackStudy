  ...
  this._renderCurrent = (idSong, isPlay = false) => {
    ...
    if (this._audioPlayer) {
      ...
      (this._audioPlayer.oncanplay = () => {
        // Hiển thị lên thanh kéo giả
        setInterval(() => {
          if (!this._audioPlayer.paused) {
            this._progressBar.value = Math.floor(
              (this._audioPlayer.currentTime / this._audioPlayer.duration) * 100
            );
          }
        }, 10);
...
const newPlayList = new PlayList(audioLists, 1);
newPlayList._init();
// });
